...
- Collection Journal 목록은 대표 카드 또는 사진, 날짜와 한 줄 기록을 중심으로 한 native-feeling list를 사용한다.
- 사진이 없더라도 카드 이미지와 문장만으로 온전한 기록이 되게 한다.
- 날짜, 장소와 함께한 사람은 기억의 맥락으로 보여주고 form 완료율이나 소비 통계로 만들지 않는다.
- 빠른 기록에서는 날짜, 한 줄 기록과 선택적 카드만 먼저 보여준다.
- 장소, 기록 유형, 사진, 기분과 긴 이야기는 `자세히 기록하기`에서 사용자가 원할 때 펼친다.
...
- 핵심 목적지는 Home, Collection, Collection Journal, Binder 네 개로 기획한다.
...
| Empty | 무엇이 없는지 설명하고 가장 관련 있는 행동 하나를 제공 |
| Error | 문제를 쉬운 문장으로 설명하고 가능한 경우 입력 보존·재시도 제공 |
| Success | 카드, 기록과 완료된 콘텐츠를 먼저 보여주고 feedback은 간결하게 제공 |
